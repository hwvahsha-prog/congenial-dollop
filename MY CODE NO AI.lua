        ...
        if v ~= LocalPlayer and v.Character and v.Character:FindFirstChild("HumanoidRootPart") then
            ...
            if on then
                ...
                if dist <= getgenv().SilentFOV and dist < shortestDist then
                    closest = v; closestPart = root; shortestDist = dist
                end
            end
        end
    end

    -- Scan Bots (if enabled)
    if getgenv().BotSupport then
        for _, model in pairs(workspace:GetDescendants()) do
            if model:IsA("Model") and model:FindFirstChild("Humanoid") and model:FindFirstChild("HumanoidRootPart") then
                local isPlayer = false
                for _, p in pairs(Players:GetPlayers()) do if p.Character == model then isPlayer = true end end
                ...
                if not isPlayer then
                    ...
                    if on then
                        local dist = (Vector2.new(sp.X, sp.Y) - center).Magnitude
                        if dist <= getgenv().SilentFOV and dist < shortestDist then
                            closest = model; closestPart = root; shortestDist = dist
                        end
                    end
                end
            end
